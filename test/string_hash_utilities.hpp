...
constexpr auto dispatch_table_from_tuple(const T& strings) {
  return dispatch_table_from_tuple_helper(strings, std::make_index_sequence<std::tuple_size<T>{}>{});
}
...
constexpr auto max_string_length_tuple(const T& strings) {
  return max_string_length_helper(strings, std::make_index_sequence<std::tuple_size<T>{}>{});
}

template<typename Table, typename StringTuple, typename Literals, size_t... I>
bool runtime_compiletime_match(
    const Table& table, const StringTuple& string_tuple, const Literals& test_strings) {
...
constexpr bool compare_single_hash(const Table& table, const StringTuple& strings, std::index_sequence<J...>&&) {
...
constexpr bool unique_hashes(const Table& table, const StringTuple& strings, std::index_sequence<I...>&&) {
  return (compare_single_hash<I>(table, strings, std::make_index_sequence<sizeof...(I)>{}) && ...);
}
